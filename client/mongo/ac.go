...
func newMongoAutoCommiter(builder *MongoAutoCommitBuilder) *MongoAutoCommitter {
	return &MongoAutoCommitter{
...
func (p *MongoAutoCommitter) Insert(doc interface{}) (e error) {
	if p.started {
		...
		errors.New(fmt.Sprintf("MongoAutoCommitter-%s(%s)_is_closed", p.name, p.coll))
	}
	return
}
...
func (p *MongoAutoCommitter) Flush() error {
	if p.verbose {
		log.Info(fmt.Sprintf("MongoAutoCommitter-%s(%s) a new flush is comming", p.name, p.coll))
	}
	...
	if p.verbose {
		log.Info(fmt.Sprintf("MongoAutoCommitter-%s(%s) a new flush is finished", p.name, p.coll))
	}
	return nil
}
...
func (p *MongoAutoCommitter) flusher(interval time.Duration) {
...
func newMongoAutoCommitWorker(p *MongoAutoCommitter, i int) *mongoAutoCommitWorker {
...
func (w *mongoAutoCommitWorker) commit() (err error) {
	...
	if w.capacity() > 0 {
		if w.p.verbose {
			log.Info(fmt.Sprintf("MongoAutoCommitter-%s(%s)-%d commiting size: %d", w.p.name, w.p.coll, w.i, len(w.docInsert)))
		}
		...
		if _, err = bulk.Run(); err != nil {
			log.Info(fmt.Sprintf("MongoAutoCommitter-%s(%s)-%d >>ERROR<<: %v", w.p.name, w.p.coll, w.i, err.Error()))
		}
		if w.p.verbose {
			log.Info(fmt.Sprintf("MongoAutoCommitter-%s(%s)-%d committed size: %d", w.p.name, w.p.coll, w.i, len(w.docInsert)))
		}
	} else {
		if w.p.verbose {
			log.Info(fmt.Sprintf("MongoAutoCommitter-%s(%s)-%d committed nothing", w.p.name, w.p.coll, w.i))
		}
	}
	return
}
